...
export async function getStaticPaths() {
    const response = await fetch("https://fast-shore-63405.herokuapp.com/posts");
...
export async function getStaticProps({params}) {
    const {slug} = params;
    console.log({slug});
    console.log('https://fast-shore-63405.herokuapp.com/posts?filters[Slug][$eq]='+{slug}.slug);
    // ?filters[username][$eq]=John
    // const res = await fetch('http://localhost:1337/api/posts?Slug=${slug}');
    const res = await fetch('https://fast-shore-63405.herokuapp.com/posts?filters[Slug][$eq]='+{slug}.slug);
