...
export default function Home({spotify_artsits, spotify_tracks}) {
  // console.log(fetchAPI("me/top/artists"))
    return (
      <>
      <h1 style={{textAlign: "center", margin-top: "5rem"}}> My Spotify Dashboard </h1>
      <div style={{ display: "flex", justifyContent: "space-evenly"}}>
        <div style={{ alginSelf: "flex-start"}}>
          ...
          <div style={{ alginSelf: "flex-end"}}>
          <h2> Top Tracks this Month </h2>
          {spotify_tracks.items 
             && spotify_tracks.items.map((track, index) => (
               <div  key={track.id}>
                      <p className={styles.myPostDesc}> {index+1}. {track.name} </p>
                </div>
            ))}
          </div>
      </div>
...
//need to: make a GET REQUEST TO THESE ENDPOINTS:
  ...
  export async function getStaticProps() {
    ...
    const spotify_artsits = await fetchAPI("me/top/artists", access_token.access_token);
    const spotify_tracks = await fetchAPI("me/top/tracks", access_token.access_token);
